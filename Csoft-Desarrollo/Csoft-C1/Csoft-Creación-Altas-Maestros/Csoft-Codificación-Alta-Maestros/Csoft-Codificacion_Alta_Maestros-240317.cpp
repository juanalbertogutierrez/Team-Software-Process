...
/*Componente:  Alta Maestros                                       */
/*Fecha: 23/03/2017                                     */
/*Descripción: Codificación que muestra el alta para maestros*/
/*Versión: 1.0                                             */
/*=========================================================*/
...
#include <cstdio>                                       /*Entrada y salida de datos, file, fflush*/
#include <cstdlib>
#include "sqlite3.h"                                    /*Interaccion base de datos*/

using namespace std;
FILE *pfile;                                            /*Archivo para imprimir las materias en la base de datos*/

/***Esta función se encarga de imprimir los datos encontrados en la base de datos***/
static int callback (void *data,int argc, char **argv,char **az_col_name){
    int i;
    printf( "%s:\n ", (const char*)data);                                   /*Imprime el tipo de consulta que esta en data*/
    for(i=0;i<argc;i++){
         printf("%s = %s\n", az_col_name[i], argv[i] ? argv[i] : "NULL");   /*Imprime cada valor de la consulta de acuerdo a su columna y valor*/
    }
    printf("\n");
    return 0;
}
...
void query_maestros(sqlite3 *db,char* id_maestro){
    ...
    int rc;
    sqlite3_stmt *res;
    sql="SELECT * FROM MAESTROS where Id_M=@Id_M;";                     /* Selecciona a los maestros con un id especifico sentencia sql*/
    ...
    sql="SELECT ID_Materia,Status FROM IMPARTE where ID_Maestro=@Id_M;";/* Instruccion sql para la seleccion de materias en tabla imparte*/
...
void insert_maestros(sqlite3 *db,char* id_maestro, char* nombre, char* ap_paterno, char* ap_materno, char* fecha_nac, char* direccion, char* telefono, char* email){
    ...
    sql="INSERT INTO MAESTROS (Id_M, Nombre, ApPaterno, ApMaterno, FNacimiento, Direccion, Telefono, Email) VALUES (@Id_M,@nom,@ap_p,@ap_m,@fec,@dir,@tel,@ema);";
    rc=sqlite3_prepare_v2(db,sql,-1,&res,0);                            /* Prepara la sentencia sql*/
    if(rc!=SQLITE_OK){
...
int main()
{
    ...
    alta_maestros(db);                                                                      /*A alta de maestros*/
    return 0;
}
